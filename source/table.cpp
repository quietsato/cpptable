...
std::string Table::innerHorizontalLine(int r)
{
    ...
    for (auto c = 0; c < this->rows[r].size(); c++)
    {
        ...
        if (c == this->rows[r].size() - 1 || r == this->rows.size() - 1 || this->getBorder(r, c)[BOTTOM] || this->getBorder(r, c + 1)[BOTTOM] || this->getBorder(r, c)[RIGHT] || this->getBorder(r + 1, c)[RIGHT])
            l += '+';
        else
            l += ' ';
    }
